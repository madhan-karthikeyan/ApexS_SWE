...
async def generate_plan(payload: PlanGenerateRequest, db: AsyncSession = Depends(get_async_db), _=Depends(require_roles("scrum_master", "product_owner"))):
    result = await db.execute(select(Sprint).where(Sprint.sprint_id == payload.sprint_id))
    sprint = result.scalar_one_or_none()
    if not sprint:
        raise HTTPException(status_code=404, detail="Sprint not found")
    result = await db.execute(
        select(DatasetUpload)
        .where(DatasetUpload.team_id == sprint.team_id)
...
async def get_plan(plan_id: str, db: AsyncSession = Depends(get_async_db)):
    result = await db.execute(select(SprintPlan).where(SprintPlan.plan_id == plan_id))
...
async def approve_plan(plan_id: str, db: AsyncSession = Depends(get_async_db), _=Depends(require_roles("scrum_master", "product_owner"))):
    result = await db.execute(select(SprintPlan).where(SprintPlan.plan_id == plan_id))
    plan = result.scalar_one_or_none()
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
...
async def export_plan(plan_id: str, format: str = "csv", db: AsyncSession = Depends(get_async_db), _=Depends(require_roles("scrum_master", "product_owner"))):
    result = await db.execute(select(SprintPlan).where(SprintPlan.plan_id == plan_id))
    plan = result.scalar_one_or_none()
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
    result = await db.execute(
        select(UserStory).where(UserStory.story_id.in_(plan.selected_stories or []))
    )
...
async def modify_plan(plan_id: str, payload: PlanModifyRequest, db: AsyncSession = Depends(get_async_db), _=Depends(require_roles("scrum_master", "product_owner"))):
    result = await db.execute(select(SprintPlan).where(SprintPlan.plan_id == plan_id))
    plan = result.scalar_one_or_none()
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
    result = await db.execute(select(Sprint).where(Sprint.sprint_id == plan.sprint_id))
    sprint = result.scalar_one_or_none()
    if not sprint:
        raise HTTPException(status_code=404, detail="Sprint not found")
    result = await db.execute(
...
async def get_all_explanations(
    ...
    stmt = select(Explanation).where(Explanation.plan_id == plan_id)
    if selected is not None:
        stmt = stmt.where(Explanation.is_selected == selected)
...
async def get_story_explanation(plan_id: str, story_id: str, db: AsyncSession = Depends(get_async_db)):
    result = await db.execute(
        select(Explanation).where(Explanation.plan_id == plan_id, Explanation.story_id == story_id)
    )
...
async def get_plan_stories(plan_id: str, db: AsyncSession = Depends(get_async_db)):
    result = await db.execute(select(SprintPlan).where(SprintPlan.plan_id == plan_id))
    plan = result.scalar_one_or_none()
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found")
